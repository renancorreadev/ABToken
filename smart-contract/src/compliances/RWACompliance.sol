...
import {NotApprovedBuyer, InvalidBuyerData, NotBoundCompliance, IncomeTooLow, ComplianceCheckFailed, ComplianceAlreadyBound, UnauthorizedCaller} from "./RWAComplianceErrors.sol";
...
contract RWAComplianceModule is AbstractModule, OwnableUpgradeable {
    ...
    modifier onlyRegisteredBuyer(address buyer) {
        if (buyers[buyer].income == 0) revert NotApprovedBuyer(buyer);
        _;
    }
    ...
    function moduleCheck(
        ...
        returns (bool)
    {
        BuyerInfo memory buyer = buyers[to];
